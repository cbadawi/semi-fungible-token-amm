...
(define-fungible-token amm-pool-token)

(define-map token-balances {token-id: uint, owner: principal} uint)
(define-map token-supplies uint uint)
(define-map token-owned principal (list 200 uint))
(define-data-var token-name (string-ascii 32) "amm-pool-token")
(define-data-var token-symbol (string-ascii 32) "amm-pool-token")
(define-data-var token-uri (optional (string-utf8 256)) (some u"")) ;; todo
...
(define-read-only (get-overall-balance (who principal))
	(ok (ft-get-balance amm-pool-token who)))
(define-read-only (get-total-supply (token-id uint))
	(ok (default-to u0 (map-get? token-supplies token-id))))
(define-read-only (get-overall-supply)
	(ok (ft-get-supply amm-pool-token)))
...
(define-read-only (get-overall-supply-fixed)
	(ok (decimals-to-fixed (ft-get-supply amm-pool-token))))
(define-read-only (get-overall-balance-fixed (who principal))
	(ok (decimals-to-fixed (ft-get-balance amm-pool-token who))))
...
(define-public (mint (token-id uint) (amount uint) (recipient principal))
	(begin
		(try! (check-is-approved))
		(try! (ft-mint? amm-pool-token amount recipient))
...
(define-public (burn (token-id uint) (amount uint) (sender principal))
	(begin
		(try! (check-is-approved))
		(try! (ft-burn? amm-pool-token amount sender))
...
(define-public (transfer (token-id uint) (amount uint) (sender principal) (recipient principal))
	(let (
		...
		(try! (ft-transfer? amm-pool-token amount sender recipient))
...
(define-private (get-balance-or-default (token-id uint) (who principal))
	...
	(define-private (check-is-approved) (contract-call? .auth check-is-approved ))