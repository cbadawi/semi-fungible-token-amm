...
(define-fungible-token wusd-m-token)

(define-constant UNDERLYING_ASSET 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.wusd)
...
(define-public (burn-scaled (amount uint) (who principal) (liquidity-index uint)) 
  (begin
    ...
    (let ((amount-scaled (div-unit-up amount liquidity-index)))
      ...
      (try! (ft-burn? wusd-m-token amount-scaled who))
      (ok true)
)))
...
(define-public (get-scaled-balance (account principal))
  (ok (ft-get-balance wusd-m-token account))
)

(define-read-only (get-scaled-total-supply)
  (ok (ft-get-supply wusd-m-token))
)
...
(define-read-only (get-total-supply)
  (ok (mul-unit (ft-get-supply wusd-m-token) (var-get normalized-income)))
)
...
(define-read-only (get-balance (account principal))
  (ok (mul-unit (ft-get-balance wusd-m-token account) (var-get normalized-income)))
)
;; @desc get-name
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (match (ft-transfer? wusd-m-token amount sender recipient)
      response (begin
        (print memo)
        (ok response)
      )
...
(begin  
    (try! (check-is-approved tx-sender))
    (ft-transfer? wusd-m-token amount liquidated-user recipient)
))
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (try! (check-is-approved tx-sender))
    ;; (ft-mint? wusd-m-token amount recipient)
    (ok false)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? wusd-m-token amount sender)
  )
)
;; ----------------------------------------------------------------------------------------
;; MATH 
