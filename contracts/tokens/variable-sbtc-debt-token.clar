...
;; non-transferable token, functions added for trait consistency
;;  todo find a cleaner solution by seperating the c tokjen trait into c tokens and variable-debt-token traits
...
(define-fungible-token variable-sbtc-debt-token)
...
(define-public (burn-scaled (amount uint) (who principal) (liquidity-index uint)) 
  (begin
    ...
    (let ((amount-scaled (div-unit-up amount liquidity-index)))
      ...
      (try! (ft-burn? variable-sbtc-debt-token amount-scaled who))
      (ok true)
)))
...
(define-public (get-scaled-balance (account principal))
  (ok (ft-get-balance variable-sbtc-debt-token account))
)

(define-read-only (get-scaled-total-supply)
  (ok (ft-get-supply variable-sbtc-debt-token))
)
...
(define-read-only (get-name)
  (ok "variable-sbtc-debt-token")
)

;; @desc get-symbol
;; @returns (response string-utf8)(define-read-only (get-symbol)
...
(define-public (mint (amount uint) (recipient principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-mint? variable-sbtc-debt-token amount recipient)
  )
)
...
(define-public (burn (amount uint) (sender principal))
  (begin
    (try! (check-is-approved tx-sender))
    (ft-burn? variable-sbtc-debt-token amount sender)
  )
)
